...
interface QuotePrintContentProps {
  ...
  setPaymentCondition: (v: string) => void;
  proposalValidity: string;
  setProposalValidity: (v: string) => void;
  deliveryTerm: string;
...
export function QuotePrintContent({
  quote,
  companyConfig,
  paymentCondition,
  setPaymentCondition,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-6 gap-4">
        <div className="flex items-center gap-4">
          {companyConfig?.logo ? (
            <img
              src={
                companyConfig.logo.startsWith('http') || companyConfig.logo.startsWith('data:')
                  ? companyConfig.logo
                  : `${api.defaults.baseURL?.replace('/api', '') || ''}${companyConfig.logo}`
              }
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {companyConfig?.website ? ` | Site: ${companyConfig.website}` : ''}
            </p>
          </div>
        </div>
        <div className="text-left md:text-right">
          ...
          <div className="mt-1 space-y-0.5 text-xs text-muted-foreground">
            <p>
              <span className="font-semibold text-slate-800">Orçamento:</span> #{quote.quoteNumber}{' '}
              (v{quote.revision})
            </p>
            <p>
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-lg p-5 print:p-4 grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-4">
        <div>
      ...
      <div className="border-t border-slate-200 pt-4 mt-4">
        ...
        <textarea
          ...
          className="w-full text-xs text-slate-700 whitespace-pre-wrap border border-dashed border-slate-300 rounded px-2 py-1 bg-white focus:outline-none focus:border-slate-500 focus:ring-0 print:border-none print:p-0 print:bg-transparent resize-none"
          rows={4}
        />
      </div>
      ...
      <div className="border-t border-slate-200 pt-16 mt-12 grid grid-cols-2 gap-8 text-center text-xs">
        <div className="space-y-2">
          ...
          <p className="font-bold text-slate-800">{companyConfig?.companyName || 'Contratada'}</p>
